...
function getCellValue(
  ...
  rowNumber: number,
  columnMap: { [columnKey: string]: number },
  columnKey: string
): string {
...
async function checkExcelFormat(filePath: string): Promise<boolean> {
  ...
  if (
    !Array.isArray(languagelistHeaders) ||
    !languagelistHeaders.includes('name') ||
    !languagelistHeaders.includes('address') ||
    !languagelistHeaders.includes('description-ch')
  ) {
  ...
  if (
    Array.isArray(languageconfigHeaders) &&
    languageconfigHeaders.includes('default') &&
    languageconfigHeaders.includes('enabled')
  ) {
    // 执行操作
  } else {
  ...
  for (let rowNumber = 2; rowNumber <= tablistRowCount; rowNumber++) {
    ...
    if (cell.text === 'list') {
      for (
        let colNumber = 2;
        colNumber <= tablistSheet.columnCount;
        colNumber++
      ) {
...
//language config
async function parseLanguageConfig(
  filePath: string,
  sheetName: string
): Promise<any[]> {
  const workbook = new ExcelJS.Workbook();
  ...
  for (let rowNumber = 2; rowNumber <= worksheet.rowCount; rowNumber++) {
    ...
    for (let colNumber = 2; colNumber <= worksheet.columnCount; colNumber++) {
      ...
      if (cell.text) {
        rowData[header] =
          cell.text === 'true' || cell.text === 'false'
            ? cell.text === 'true'
            : cell.text;
      }
    }
...
async function parseLanguageList(
  filePath: string,
  sheetName: string
): Promise<{ ch: Map<string, string>; en: Map<string, string> }> {
  const workbook = new ExcelJS.Workbook();
  await workbook.xlsx.readFile(filePath);
...
//tablist
async function parseTablist(
  filePath: string,
  sheetName: string,
  languageListSheetName: string
  ...
  for (
    let columnNumber = 1;
    columnNumber <= languageSheet.columnCount;
    columnNumber++
  ) {
  ...
  for (let colNumber = 2; colNumber <= columnCount; colNumber += 1) {
    ...
    for (let rowNumber = 4; rowNumber < worksheet.rowCount; rowNumber += 4) {
      ...
      if (listSheetName.replace(/"/g, '').trim().length == 0) {
        console.log(
          '[Tablist]list field is null,but subtitle is exist:' +
            subTabTitleCell.value
        );
      } else {
        const listSheet = workbook.getWorksheet(
          listSheetName.replace(/"/g, '')
        );
        if (!listSheet) {
        ...
        for (
          let columnNumber = 1;
          columnNumber <= listSheet.columnCount;
          columnNumber++
        ) {
        ...
        for (let rowNumber = 2; rowNumber <= listSheet.rowCount; rowNumber++) {
          ...
          //options
          const optionsText = getCellValue(
            listSheet,
            rowNumber,
            columnMap,
            'options'
          ...
          if (nameCol && addressCol) {
            ...
            const tagRow =
              languageSheet.getColumn(nameCol).values.indexOf(tagName) + 1;
            if (tagRow > 0) {
...
//# single table
async function parseSingleTable(
  filePath: string,
  ...
  for (const worksheet of workbook.worksheets) {
    if (worksheet.name.startsWith('#')) {
      ...
      for (const contentArea of contentAreas) {
        ...
        for (
          ...
          while (true) {
            ...
            switch (columnValueTitleCell.text) {
              case 'title':
                ...
                contentItem[columnValueTitleCell.value as string] =
                  worksheet.getCell(contentStartRow, colNumber).text;
                break;
              case 'options':
                //options
                const optionsText = worksheet.getCell(
                  contentStartRow,
                  colNumber
                ).text;
                const optionsMatch = optionsText.match(/\["(.*?)",(\s*.*?)?]/g);
                ...
                break;
              case 'type':
                const typeVal = worksheet.getCell(
                  contentStartRow,
                  colNumber
                ).text;
                if (typeVal == 'title') {
                  ...
                  contentItem['type'] = worksheet.getCell(
                    contentStartRow,
                    colNumber
                  ).text;
                } else {
          ...
          if (nameCol && addressCol) {
            ...
            const tagRow = languageSheet
              .getColumn(nameCol)
              .values.indexOf(tagName);
      ...
      exportJsonToFile(
        singleTableSheetData,
        targetJsonDir + '/' + jsonFileName
      );
    }
  }
}
